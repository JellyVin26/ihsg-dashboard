...
function setLoading(on) {
  ...
  document.getElementById('loadBtn').textContent = on ? 'Loading…' : 'Analyze';
}
...
function renderPriceHeader(data) {
  ...
  setText('tickerLabel',  data.yahoo_symbol ?? data.ticker ?? '—');
  setText('priceDisplay', latest.toLocaleString('id-ID', { maximumFractionDigits: 0 }));

  const chEl = document.getElementById('changeDisplay');
  if (chEl) {
    chEl.textContent = `${isUp ? '+' : ''}${change.toFixed(0)} (${isUp ? '+' : ''}${changePct.toFixed(2)}%)`;
    chEl.className   = 'change ' + (isUp ? 'change--up' : 'change--down');
  }

  setText('dataNote', data.isDemo
    ? '⚠ Demo data — start server.py for live prices'
    : `✓ Live · updated ${new Date().toLocaleTimeString()}`);
  setStyle('dataNote', 'color', data.isDemo ? '' : 'var(--color-green)');
}

function renderMetrics(prices, data) {
  ...
  const signals = [
    ...
    },
    {
      ...
      action: lastBB.lower && last < lastBB.lower ? 'buy'
            ...
            : 'neutral',
      desc: lastBB.lower && last < lastBB.lower ? 'Below lower band'
          : lastBB.upper && last > lastBB.upper ? 'Above upper band'
          : 'Inside bands',
    },
    {
      name: 'Price vs MA50',
    ...
    },
    {
      ...
      desc: upDays >= 3 ? 'Short-term uptrend' : upDays <= 1 ? 'Short-term downtrend' : 'Sideways',
    },
  ];
...
function buildCharts(prices, labels) {
  ...
  const baseAxis  = { ticks: { color: tickColor, font: { size: 11 } }, grid: { color: gridColor } };
  const baseOpts  = {
    responsive: true, maintainAspectRatio: false, animation: false,
    plugins: { legend: { display: false }, tooltip: { mode: 'index', intersect: false } },
    scales: { x: { ...baseAxis, ticks: { ...baseAxis.ticks, maxTicksLimit: 8 } }, y: baseAxis },
  };

  const priceSets = [{
    label: 'Price', data: prices, borderColor: '#185FA5',
    backgroundColor: 'rgba(24,95,165,0.06)', borderWidth: 1.8,
  ...
  if (state.indicators.bb) {
    priceSets.push({
      label: 'BB Upper', data: bb.map(b => b.upper), borderColor: '#B4B2A9',
      borderWidth: 0.8, borderDash: [2,3], pointRadius: 0,
      fill: '+1', backgroundColor: 'rgba(136,135,128,0.05)', tension: 0.3, order: 1,
    });
    priceSets.push({
      label: 'BB Lower', data: bb.map(b => b.lower), borderColor: '#B4B2A9',
      borderWidth: 0.8, borderDash: [2,3], pointRadius: 0, fill: false, tension: 0.3, order: 1,
    });
  }

  state.charts.price = new Chart(document.getElementById('priceChart'), {
    type: 'line', data: { labels, datasets: priceSets }, options: { ...baseOpts },
  });

  state.charts.rsi = new Chart(document.getElementById('rsiChart'), {
    type: 'line',
    data: { labels, datasets: [{ label: 'RSI', data: rsiV, borderColor: '#534AB7', borderWidth: 1.5, pointRadius: 0, fill: false, tension: 0.3 }] },
    options: { ...baseOpts, scales: {
      x: { ...baseAxis, ticks: { ...baseAxis.ticks, maxTicksLimit: 6 } },
      y: { ...baseAxis, min: 0, max: 100 },
    }},
  });
  ...
  state.charts.macd = new Chart(document.getElementById('macdChart'), {
    ...
    data: { labels, datasets: [
      { type: 'line', label: 'MACD', data: macdLine, borderColor: '#1D9E75', borderWidth: 1.5, pointRadius: 0, fill: false, tension: 0.3, order: 1 },
      { type: 'line', label: 'Signal', data: signalLine, borderColor: '#D85A30', borderWidth: 1.2, borderDash: [4,3], pointRadius: 0, fill: false, tension: 0.3, order: 1 },
      { label: 'Histogram', data: histogram, backgroundColor: histogram.map(v => v >= 0 ? 'rgba(29,158,117,0.45)' : 'rgba(216,90,48,0.45)'), borderWidth: 0, order: 2 },
    ]},
    options: { ...baseOpts, scales: {
      x: { ...baseAxis, ticks: { ...baseAxis.ticks, maxTicksLimit: 6 } },
      y: baseAxis,
...
async function loadStock() {
  ...
  try {
    ...
    console.warn('Live data unavailable, falling back to demo:', err.message);
    data = generateDemoData(state.ticker, state.period);
    state.usingLiveData = false;
    setStatus(
      'Server not running — showing demo data. Start server.py to see real prices.',
      'warn'
    );
  }
